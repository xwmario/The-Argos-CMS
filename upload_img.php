...
$page_title = array('page_title'=>$lang_sys['lang_upload_img']); //title на страницата
...
if (strpos($_SERVER["REQUEST_URI"], '/upload_img.php') !== false) {
	$is_uploadimg_page = array('is_uploadimg_page'=>1);
}
//end
 
/////////PRINT/////// 
$tpl = $mustache->loadTemplate('upload_img');  //име на темплейт файла
echo $tpl->render($page_title + $values + $values3 + $values4 + $lang_sys + $banner88x31 + $banner468x60 + $sliders + $is_uploadimg_page); //принтираме всичко в страницата ($values+$values)